 ...
 * 快照：v2.1-snapshot-1…（挂靠下一个正式版）；当前开发线 v2.2-snapshot-1
 * 简单视图 = 大厂更新日志式重点；详细视图 = Minecraft 式（新增/修改/Bug 修复）
 */
(function () {
  var VERSIONS = [
    {
      v: "v2.1.1", theme: "白皮书馆 · 正式版", date: "2026-08-17", kind: "patch",
      simple: "2.1 转正：快照期修复全部合入；城市探索封面显示完整城市名；移动端滚动与初始化白屏修复；无新功能",
      detail: {
        added: [],
        changed: [
          "快照 v2.1-snapshot-1 内容转正为 v2.1.1 正式版，本地数据无缝保留",
          "志愿日程取消独立选项卡，并入工作台卡片（最近待办 + 上一/下一小卡 + 全程二级弹层）"
        ],
        fixed: [
          { id: "BUG-206", desc: "移动端上下滑动全部失效", fix: "恢复视口锁定 + 内容区独立滚动，个人中心同规则" },
          { id: "BUG-207", desc: "初始化白屏（变量提前引用导致崩溃）", fix: "修复 renderNext 声明顺序；静态资源加缓存版本号防旧缓存" },
          { id: "BUG-208", desc: "城市探索封面城市名截断为两字（六盘水→六盘）", fix: "封面显示完整城市名并按字数自适应字号" }
        ]
      }
    },
    {
